          ...
          <template v-else>
            ...
            <div class="flex flex-row gap-2 justify-center w-full">
              <button
                ...
                :class="{ 'text-white/30': char === emptyChar, 'animate-blink': idx === position }"
                @click="position = idx"
              >
                {{ char }}
              </button>
...
const barcodeChars = computed(() => barcode.value.split('').slice(0, BARCODE_LENGTH));
const isEmptyBarcode = computed(() => barcodeChars.value.every((char) => char === emptyChar));
const isValidBarcode = computed(() => /^\d{6}$/.test(barcodeChars.value.join('')));
const cannotBackspace = computed(() => position.value === 0 && barcodeChars.value[position.value] === emptyChar);
...
async function loadStoredBarcode() {
  ...
  if (value) {
    barcode.value = value.padEnd(BARCODE_LENGTH, emptyChar).slice(0, BARCODE_LENGTH);
    await renderBarcode();
  } else {
...
async function renderBarcode() {
  position.value = barcode.value === initialBarcode ? 0 : barcode.value.length - 1;
